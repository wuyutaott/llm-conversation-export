...
def fetch(url, method="GET", body=None, headers=None, credentials=False):
    """页面内 fetch，返回 (status, text)；遇 429 自动退避重试。"""
    ...
    for attempt in range(1, MAX_RETRY + 1):
        ...
        if res["s"] == 429:
            print(f"    429 限流，第 {attempt}/{MAX_RETRY} 次重试前等 {delay:.0f}s ...")
            time.sleep(delay); delay = min(delay * 2, 90); continue
        return res["s"], res["t"]
    raise RuntimeError("多次重试仍被限流")
...
def download(url, headers=None, credentials=False):
    """下载二进制资源，返回 (bytes, content_type)；遇 429 自动退避。"""
    ...
    for attempt in range(1, MAX_RETRY + 1):
        ...
        if res["s"] == 429:
            print(f"      429 限流（下载），等 {delay:.0f}s ...")
            time.sleep(delay); delay = min(delay * 2, 90); continue
