...
def navigate(url, wait=5):
    """在当前标签页整页跳转（不新开标签）。"""
    _H["cdp"]("Page.navigate", url=url)
    try:
        _H["wait_for_load"]()
    except Exception:
        pass
...
def _opts_js(method, headers, body, credentials):
    ...
    if body is None:
        ...
        body_line = "o.body=%s;" % json.dumps(body)               # 原样字符串体（如表单 f.req=...）
    else:
        body_line = "o.body=JSON.stringify(%s);" % json.dumps(body)  # dict/list 体转 JSON
