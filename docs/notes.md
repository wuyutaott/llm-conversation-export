...
- 序号规则：**越新（创建越晚）的会话序号越大**，最旧 = 001。新会话在末尾追加更大的号，
  已有会话的序号永久固定——即使会话被重新激活（update_time 变化）也不动。这样增量导出
  无需整体重排，只把新会话补在后面。`storage.write_titles` 按「创建时间」升序给新会话分配
  `当前最大序号+1`，已有会话沿用 csv 里记录的旧序号。
- 改了渲染逻辑想刷新历史导出：`python3 rerender.py`（用本地 JSON 重渲染，不重爬，图片链接就地复用）。
- 一次性把旧编号（曾经 001=最新）迁移到新规则：`python3 renumber.py`（先预览）→ `--apply` 执行，
  之后务必再跑一次 `rerender.py` 修复 markdown 里的图片相对路径。
